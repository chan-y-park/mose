...
def sample_path_data(dLoc):
    ...
    initPoint = 1j*(min_y-2*spacing)+(dLoc[0].real-2*spacing)
...
def monodromy_at_point_via_path(init_root,d,f,g,path_data,\
                            ...
                            rotation_recursion_level=0, rotation_n=8):
    """
    ...
    if control_var == 'fine':
        ...
        r_init_root = [x / (zeta) for x in init_root]
...
if __name__=="__main__":
    ...
    #Seiberg-Witten
    # zeta = np.exp(np.pi*1j/8.0)
    zeta = np.exp(0)
    wmodel=WeierstrassProto([(-1.0/3) / (zeta**2), 0, (1.0/4.0) / (zeta**2)],[(-2.0/27) / (zeta**3), 0, (1.0/12) / (zeta**3), 0])
    ...
    mon = monodromy_at_point_wmodel(dnum,wmodel,10000,10000,option='p')
    print mon

    from elliptic_fibration import monodromy_eigencharge
    print monodromy_eigencharge(mon)
