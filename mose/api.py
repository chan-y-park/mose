...
def set_logging(level, logging_file_name=None):
    if level == 'debug':
...
def analysis(config, phase=None,):
    warnings.warn(
        'mose.api.analysis() is deprecated, '
        'use mose.api.generate_k_wall_network().'
    )
    return generate_k_wall_network(config, phase)


def generate_k_wall_network(config, phase=None,):
    data = {
...
def load(data_dir=None):
    warnings.warn(
        'mose.api.load() is deprecated, '
        'use mose.api.load_k_wall_network().'
    )
    return load_k_wall_network(data_dir)


def load_k_wall_network(data_dir=None):
    """
...
def save(
    config, data, k_wall_network_plot=None, ms_wall_plot=None,
    data_dir=None, open_dialog=False
):
    warnings.warn(
        'mose.api.save() is deprecated, '
        'use mose.api.save_k_wall_network().'
    )
    return save_k_wall_network(
        config, data, k_wall_network_plot, ms_wall_plot,
        data_dir, open_dialog
    )

def save_k_wall_network(
    config, data, k_wall_network_plot=None, ms_wall_plot=None,
    data_dir=None, open_dialog=False, logging_file_name=None,
):
    """
