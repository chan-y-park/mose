...
class WeierstrassModel:
    ...
    def __init__(self,fcoeff,gcoeff):
        ...
        print "These are the f,g functions:\nf = %s\ng = %s" % (self.f, self.g)    
...
#        return [sorted_rts, max_diff_cons]
    ...
    def braiding_action(old_rts,new_rts):
        ...
        if i_old==i_new:
            ...
            return 'zoom'
            # print "old indices: %s" % i_old
            # print "new indices: %s" % i_new
            # print "old roots: %s" % old_rts
            # print "new roots: %s" % new_rts
    ...
    def solve_diff_eq(timesteps, f, g, vector, start, last_roots, min_diff_rts, G, max_diff_cons, recursion_level):
        # print 'parameters for diff eqn: \n %s' % \
        #     [time, f, g, vector, start, last_roots, min_diff_rts, G]
        rec_lvl = recursion_level+1
        print 'Entering recursion level %s' % rec_lvl
        sequence = [start]
        delta_u = vector / timesteps
        for t in range(timesteps):
            new_point = start + (t+1) * delta_u
            poly = np.poly1d([1,0,f(new_point),g(new_point)])
            roots = poly.r
            # print 'time: %s roots: %s' % (t, roots)
            #print 'Time : '+str(t)
            #print 'Roots : '+str(roots)
            roots, cons = sort_roots(roots,last_roots)
            if is_degenerate(roots):
            ...
            if b!='I' and b!='zoom':
                print 'Found braiding! %s' % b
                G.append(b)
                sequence.append(new_point)
            elif b == 'zoom':
                z_factor = 5
                z_timesteps = z_factor
                # z_time = np.linspace(0,1.,z_factor+1)
                # z_delta_t = delta_t / z_factor
                z_vector = delta_u
                z_start = start + t * delta_u
                print 'Zooming in! \n\tnew vector: %s\n\tnew start: %s' % (z_vector, z_start)
                print 'sequence: \n%s' % sequence[-2:]


                G, min_diff_rts, max_diff_cons, last_roots, new_sequence = \
                solve_diff_eq(z_timesteps, f, g, z_vector, z_start, \
                                last_roots, min_diff_rts, G, max_diff_cons, rec_lvl)
                sequence += new_sequence
            # raise ValueError('just roots')
        ...
        return [G, min_diff_rts, max_diff_cons, last_roots, sequence]
    ...
    G, min_diff_rts, max_diff_cons, last_roots, sequence = \
                solve_diff_eq(timesteps, f, g, vector, start, \
                                last_roots, min_diff_rts, G, max_diff_cons, rec_lvl)
    print "\t\t*** THESEQUENCE ***\n%s" % sequence
